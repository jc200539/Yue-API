...
import com.yue.apicommon.utils.CookieUtil;
...
//import com.yue.project.model.dto.*;
import com.yue.project.model.dto.user.*;
import com.yue.apicommon.model.entity.User;
import com.yue.project.model.vo.UserDevKeyVO;
import com.yue.project.model.vo.UserVO;
...
public class UserController {
    ...
    public Long getUserIdByCookie(HttpServletRequest request){
        String cookie = com.yue.project.util.CookieUtil.getCookieValue(request,USER_LOGIN_KEY);
    ...
    public BaseResponse<User> userLogin(@RequestBody UserLoginRequest userLoginRequest, HttpServletResponse response) throws NoSuchAlgorithmException {
        ...
        redisUtil.set("session:" + newToken, JSONUtil.toJsonStr(user), 6000); // token + userInfo
    ...
    public BaseResponse<Boolean> updateUser(@RequestBody UserUpdateRequest userUpdateRequest, HttpServletRequest request, HttpServletResponse response) {
        if (userUpdateRequest == null || getUserIdByCookie(request) < 0) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR);
        }
        User user = new User();
        BeanUtils.copyProperties(userUpdateRequest, user);
        user.setId(getUserIdByCookie(request));
        ...
        String userJson = (String) redisUtil.get("session:" + loginToken);
        //json转换成Use对象
        User temp = JSONUtil.toBean(userJson, User.class);
